...
namespace unity.libsodium
{
    ...
    public static class NativeLibsodium
    {
        ...
        public static extern int crypto_aead_chacha20poly1305_encrypt(
			byte[] cipher, out long cipherLength, byte[] message, long messageLength, byte[] additionalData,
		...
		[DllImport(DLL_NAME)] public static extern int crypto_aead_xchacha20poly1305_ietf_encrypt(byte[] c,
			out long clen,
			byte[] m,
			long mlen,
			byte[] ad,
			...
			byte[] npub,
			byte[] k);
		[DllImport(DLL_NAME)] public static extern int crypto_aead_xchacha20poly1305_ietf_decrypt( byte[] m,
			out long mlen,
			byte[] nsec,
			byte[] c,
			long clen,
