...
namespace Coflnet {
	...
	*/
		...
		public static byte[] DecryptData (byte[] ciphertextWithNonce, byte[] key) {
			...
			if (result != 0)
				throw new DecrytpionError ();

			return message;
		}

		public class DecrytpionError : Exception {
			public DecrytpionError (string message = "An error occured while decrypting") : base (message) { }
		}

		public class EncryptionError : Exception {
			public EncryptionError (string message = "An error occured while encrypting") : base (message) { }
		}
